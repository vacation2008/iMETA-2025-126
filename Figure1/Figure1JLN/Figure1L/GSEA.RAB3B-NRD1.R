...
setwd("C:\\Users\\cikeli\\Desktop\\0213\\gsea\\RAB3B-NRD1.GSEA")     
...
risk <- read.delim("C:/Users/cikeli/Desktop/0213/gsea/RAB3B-NRD1.GSEA/risklandscap.RAB3B-NRD1.tsv")
...
write.table(kkTab,file="RAB3B-NRD1.txt",sep="\t",quote=F,row.names = F)


termNum=1     
kkUp=kkTab
if(nrow(kkUp)>=termNum){
  rows <- c("KYNG_DNA_DAMAGE_BY_UV")
  selected_rows <- kkUp[row.names(kkUp) %in% rows, ]
  showTerm=row.names(selected_rows)[1:1]     
  gseaplot=gseaplot2(kk, showTerm, base_size=8, title="KYNG_DNA_DAMAGE_BY_UV")
  pdf(file="GSEA.RAB3B-NRD1.pdf", width=7, height=5.5)
  print(gseaplot)
  dev.off()
}
